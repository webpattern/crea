...
<!--//Шапка с логотипом и меню-->	
        ...
        <div class="page-container"> 
			...
			<li typeof="v:Breadcrumb">
			<a rel="v:url" property="v:title" href="signup.php">Регистрация</a>
			<span class="divider"></span>
			</li>
			</ul>
	...
	if( isset($data['do_signup']))
	{
		...
		if( trim($data['login']) == '' )
		{
			$errors[] = '<div style="color: red; font-size: 14px;">Введите логин</div>';
 		}
		
		if( trim($data['email']) == '' )
		{
			$errors[] = '<div style="color: red; font-size: 14px;">Введите Email</div>';
 		}
		
		if($data['password'] == '' )
		{
			$errors[] = '<div style="color: red; font-size: 14px;">Введите пароль</div>';
 		}
		
		if($data['password_2']  != $data['password'] )
		{
			$errors[] = '<div style="color: red; font-size: 14px;">Павторный пароль введен неверно</div>';
 		}
		
		if( R::count('users', "login = ?", array( $data['login'])) > 0)
		{
			$errors[] = '<div style="color: red; font-size: 14px;">Пользователь с таким логином уже существует!</div>';
		}
			
		if( R::count('users', "email = ?", array( $data['email'])) > 0)
		{
			$errors[] = '<div style="color: red; font-size: 14px;">Пользователь с таким Email уже существует!</div>';
		}
		...
		if(  empty($errors)  )
		{
			...
			R::store($user);
				echo '<div style="color: green; font-size: 14px;">Вы успешно зарегистрированы!</div><hr>';
 		} else
		{
			echo '<div style="color: red; font-size: 14px;">' .array_shift($errors) .'</div><hr>';
		}
...
<style>
	.log_title
	{
		...
		font-size: 16px; color: #ffffff; position: inherit; line-height: 15px;border-bottom: 0px;padding: 8px 14px; background-color: #828282;
		text-align: center; margin-left: 25%; width: 50%;"
	}
</style>

<div class="col-md-9 col-sm-9 col-xs-6" style="width: 40%;
    margin-left: 30%;
    margin-right: 30%;"> 
<form action="signup.php" method="POST">

<p>
		<p class="log_title"><strong> Ваш логин</strong></p>
...
<p>
		<p class="log_title"><strong> Ваш Email</strong></p>
		<input style="width: 100%; height: 40px; box-shadow: inset 0px 0px 11px #664d37; font-size: 20px;" type="email" name="email" value="<?php echo @$data['email']; ?>">
</p>

<p>
		<p class="log_title"><strong> Ваш пароль</strong></p>
...
<p>
		<p class="log_title"><strong> Введите пароль еще раз</strong></p>
		<input style="width: 100%; height: 40px; box-shadow: inset 0px 0px 11px #664d37; font-size: 20px;" type="password" name="password_2" value="<?php echo @$data['password_2']; ?>">
</p>

<p>
	<button style="width: 70%;height: 40px;margin-top: 20px;margin-left: 15%; background: #ce9054; border: 1px solid #cccccc; border-radius: 2px;  
	text-transform: uppercase; color: #fff; text-shadow: 0px -1px 1px black; font-weight: 600; font-size: medium;"
	type="submit" name="do_signup">Зарегистрироватся</button>
</p>

</form>

<span class="loginBlock text-center" style="margin-left: 45%;"><a href="login.php">Войти</a></span> 
